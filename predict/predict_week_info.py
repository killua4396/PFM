...
def predict_week_info():
    ...
    def get_today_df(df,arr,pf):
        return [get_Today(df),pf,get_dayofweek(arr),get_dateT(arr),get_pre_date_flow(df),get_MA5(df),get_MA10(df),get_isMonday(df),
    ...
    #写入数据库
    from database import predict_weekinfo
    db.session.query(predict_weekinfo).delete()
    db.session.commit()
    predict_W1 = predict_weekinfo(Week="This_W", Weekday_PassengerFlow = t_weekday_pf,Weekend_PassengerFlow=t_weekend_pf)
    predict_W2 = predict_weekinfo(Week="Next_W", Weekday_PassengerFlow = n_weekday_pf,Weekend_PassengerFlow=n_weekend_pf)
    db.session.add_all([predict_W1,predict_W2])
    db.session.commit()
