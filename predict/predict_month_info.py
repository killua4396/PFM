...
import xgboost as xgb
#经过调参过后的参数
other_params = {'learning_rate': 0.1, 'n_estimators': 700, 'max_depth': 3, 'min_child_weight': 6, 'seed': 0,
                'subsample': 0.6, 'colsample_bytree': 0.9, 'gamma': 0.1, 'reg_alpha': 0.1, 'reg_lambda': 1}
...
num2 = cursor.execute(sql2,args1)
if num2>0:
    conn.commit()
else:
    conn.rollback()

num3 = cursor.execute(sql3,args2)
if num2>0:
    conn.commit()
else:
    conn.rollback()
